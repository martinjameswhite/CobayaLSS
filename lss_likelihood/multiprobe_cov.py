...
class MultiprobeCovariance:
    def __init__(self, spectrum_info, noise_info):
        ...
        self.n_zbin0_spectra = dict(
            zip(
                self.spectrum_types,
                [len(self.spectrum_types[s]["bins0"]) for s in self.spectrum_types],
            )
        )
        self.n_zbin1_spectra = dict(
            zip(
                self.spectrum_types,
                [len(self.spectrum_types[s]["bins1"]) for s in self.spectrum_types],
            )
        )
        self.n_bins_spectra = dict(
            zip(
                self.spectrum_types,
                [
                    self.n_zbin0_spectra[s] * self.n_zbin1_spectra[s]
                    for s in self.spectrum_types
                ],
            )
        )
        self.n_sep_spectra = dict(
            zip(
                self.spectrum_types,
                [self.spectrum_types[s]["n_dv_per_bin"] for s in self.spectrum_types],
            )
        )
    ...
    def compute_covariance(self):
        ...
        for t0, t1 in product(self.spectrum_types, self.spectrum_types):
            ...
            if (t0[0] == "p") & (t1[0] == "p"):
                ...
                raise (
                    ValueError(
                        "Covariance computation for spec types {}-{} not implemented".format(
                            t0, t1
                        )
                    )
                )
            ...
            for zb00, zb01, zb10, zb11 in product(n00, n01, n10, n11):
                ...
                for ti, t in enumerate([t0, t1]):
                    ...
                    if (t[0] == "p") | (t[3] == "d") | (t[3] == "c"):
                        ...
                        m = self.spectrum_info[t][
                            "{}_{}_model".format(zbins[0], zbins[1])
                        ]
                        models.append(m)
...
def get_H(k, l_max=20, lowring=False):
    ...
    ell = get_ell(l_max)
    return np.stack(
        [P2xi(k, l=l, N=len(k), lowring=lowring).matrix(full=False)[2] for l in ell],
        axis=0,
    )
...
def get_Qllkk(k, sin, Q, H, l_max, lp_max, l_Q_max, lowring=False):
    ...
    for j, lp in enumerate(ell_prime):
        for i, l in enumerate(ell):
            # Q[i, j] = Q[i, j] @ H[j]
            _, Qllkk[i, j] = P2xi(k, l=lp, N=len(k), lowring=lowring)(
                Qllkk[i, j], axis=1, extrap=False, convonly=True
            )
...
def get_Cllkk_cross(
    ...
    Qllkk_w_acbd = get_Qllkk(
        k, si, Q_w_acbd, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    Qllkk_w_adbc = get_Qllkk(
        k, si, Q_w_adbc, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )

    Qllkk_x_acbd = get_Qllkk(
        k, si, Q_x_acbd, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    Qllkk_x_bdac = get_Qllkk(
        k, si, Q_x_bdac, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    Qllkk_x_adbc = get_Qllkk(
        k, si, Q_x_adbc, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    Qllkk_x_bcad = get_Qllkk(
        k, si, Q_x_bcad, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )

    Qllkk_s_acbd = get_Qllkk(
        k, si, Q_s_acbd, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    Qllkk_s_adbc = get_Qllkk(
        k, si, Q_s_adbc, H, l_max + l_P_max, l_max + l_P_max, l_Q_max
    )
    ...
    for i in range(Qllkk_x_acbd.shape[0]):
        C += (
            Pll_ac[:, i][:, None, :, None] * Qllkk_x_acbd[[i]]
            + Pll_ac[:, i][None, :, None, :] * Qllkk_x_acbd[[i]]
        ) / 2
        C += (
            Pll_bd[:, i][:, None, :, None] * Qllkk_x_bdac[[i]]
            + Pll_bd[:, i][None, :, None, :] * Qllkk_x_bdac[[i]]
        ) / 2
        C += (
            Pll_ad[:, i][:, None, :, None] * Qllkk_x_adbc[[i]]
            + Pll_ad[:, i][None, :, None, :] * Qllkk_x_adbc[[i]]
        ) / 2
        C += (
            Pll_bc[:, i][:, None, :, None] * Qllkk_x_bcad[[i]]
            + Pll_bc[:, i][None, :, None, :] * Qllkk_x_bcad[[i]]
        ) / 2
...
def get_cell_with_noise(ell_th, ell, c_ell, n_ell, spin_a, spin_b):
    """
    Always dealing with fields whose B modes are basically zero,
    so set EB, and BB c_ells to zero
    """
    c_ell_w_n = c_ell + n_ell
    c_ell_w_n = interp1d(ell_th, c_ell_w_n, fill_value="extrapolate")(ell)

    if (spin_a == 0) & (spin_b == 0):
        return [c_ell_w_n]
    elif ((spin_a == 0) & (spin_b == 2)) | ((spin_b == 0) & (spin_a == 2)):
        return [c_ell_w_n, np.zeros_like(c_ell_w_n)]
    elif (spin_a == 2) & (spin_b == 2):
        return [
            c_ell_w_n,
            np.zeros_like(c_ell_w_n),
            np.zeros_like(c_ell_w_n),
            np.zeros_like(c_ell_w_n),
        ]


def get_fields_and_coupling_mats(nside, delta_ell, ell_start, field, masks):

    ells = np.arange(3 * nside, dtype="int32")  # Array of multipoles
    weights = 1 / delta_ell * np.ones_like(ells)  # Array of weights
    bpws = -1 + np.zeros_like(ells)  # Array of bandpower indices
    i = 0
    while delta_ell * (i + 1) + 2 < 3 * nside:
        bpws[delta_ell * i + ell_start : delta_ell * (i + 1) + ell_start] = i
        i += 1

    lbins = nmt.NmtBin(nside=nside, bpws=bpws, ells=ells, weights=weights)
    ell_eff = lbins.get_effective_ells()
    n_ell = len(ell_eff)

    field = fitsio.read(field)
    if len(masks) == 1:
        mask = fitsio.read(masks[0])

        fields = [
            nmt.NmtField(mask, [field]),
            nmt.NmtField(mask, [field]),
            nmt.NmtField(mask, [field]),
            nmt.NmtField(mask, [field]),
        ]
        ...
        fields = []
        for i in range(4):
            mask = fitsio.read(masks[i])
            fields.append(nmt.NmtField(mask, [field]))

    wsp_ab = nmt.NmtWorkspace()
    wsp_ab.compute_coupling_matrix(fields[0], fields[1], lbins)
    wlab = wsp_ab.get_bandpower_windows()

    wsp_cd = nmt.NmtWorkspace()
    wsp_cd.compute_coupling_matrix(fields[2], fields[3], lbins)
    wlcd = wsp_ab.get_bandpower_windows()

    return fields, wsp_ab, wsp_cd, wlab, wlcd, ells
...
def get_angular_Cll(
    ...
    field=None,
    masks=None,
    coupling_mat_ab=None,
    coupling_mat_cd=None,
    coupling_coeff=None,
):

    if (coupling_mat_ab == None) | (coupling_mat_cd == None):
        ...
        (
            ...
            wlcd,
            ells
        ) = get_fields_and_coupling_mats(nside, delta_ell, ell_start, field, masks)
        
    n_ell = len(ells)
    
    if coupling_coeff is None:
        coupling_coeff = nmt.NmtCovarianceWorkspace()
        coupling_coeff.compute_coupling_coefficients(*fields)
        
    cell_ad_w_n = get_cell_with_noise(ell_th, ells, cell_ad, nell_ad, spin_a, spin_d)
    cell_bc_w_n = get_cell_with_noise(ell_th, ells, cell_bc, nell_bc, spin_b, spin_c)
    cell_ac_w_n = get_cell_with_noise(ell_th, ells, cell_ac, nell_ac, spin_a, spin_c)
    cell_bd_w_n = get_cell_with_noise(ell_th, ells, cell_bd, nell_bd, spin_b, spin_d)
    ...
    covar = covar.reshape([n_ell, spin_a * spin_b, n_ell, spin_c * spin_d])

    return covar, coupling_mat_ab, coupling_mat_cd, wlab, wlcd, coupling_coeff
