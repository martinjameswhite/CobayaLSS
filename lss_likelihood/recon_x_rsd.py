...
#                         (cov_raw['zbin11'] == zb11)))
    ...
    def compute_bao_pkmu(self, mu_obs, bao_sample_name, Zels):
        '''
    ...
    def bao_predict(self,bao_sample_name, Zels):
        ...
        for ii, nu in enumerate(nus_calc):
            pknutable[ii,:] = self.compute_bao_pkmu(nu, bao_sample_name, Zels)
    ...
    def logp(self, **params_values):
        ...
        if self.compute_xiell:
            
            if not self.xiell_emulators:
                pt_models = self.provider.get_result('pt_recon_zel_mod')
                
            for i in range(self.ndbins):
                if i not in self.use_samples:
                    continue
                    
                bao_sample_name = str(i)
                
                if not self.pell_emulators:
                    Zels = pt_models[i]
                    rr0, xi0t, xi2t = self.bao_predict(bao_sample_name, Zels)
                    
                    # We don't need the whole range
                    rrange = (rr0 > 60) * (rr0 < 150)
                    rr0 = rr0[rrange]
                    xi0t = xi0t[rrange]
                    xi2t = xi0t[rrange]
                    
                if not hasattr(self, 'xiells'):
                    self.xiells = np.zeros((self.ndbins, len(rr0), 2))

                self.xiells[i, :, 0] = xi0t
                self.xiells[i, :, 1] = xi2t
