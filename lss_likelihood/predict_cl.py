...
#                self.smag[i] * 1.5 * (self.OmM) / 2997.925**2 * (1 + self.zval) * g
    ...
    def compute_galaxy_convergence_kernels(self, Nchi=101):
        ...
        for i in range(self.n_source):
            dndz_n = np.interp(zvalp, self.zz, self.dndz_source[i], left=0, right=0)
            Ez = self.E_of_z(zvalp)
            g = (chivalp - self.chival[np.newaxis, :]) / chivalp
            g *= dndz_n * Ez / 2997.925
            g = self.chival * simps(g, x=chivalp, axis=0)
    ...
    def compute_galaxy_density_kernels(self, Nchi=101):
        ...
        for i in range(self.n_lens):
            ...
            self.zeff_d[i] = simps(
                self.zval * self.w_d[i] ** 2 / self.chival**2, x=self.chival
            )
            self.zeff_d[i] /= simps(self.w_d[i] ** 2 / self.chival**2, x=self.chival)
    ...
    def __init__(
        ...
        if self.dndz_lens is not None:
            ...
            for i in range(self.n_lens):
                self.dndz_lens[i] = self.dndz_lens[i] / simps(
                    self.dndz_lens[i], x=self.zz, axis=-1
                )
        else:
            self.n_lens = 0
        ...
        if self.dndz_source is not None:
            ...
            for i in range(self.n_source):
                self.dndz_source[i] = self.dndz_source[i] / simps(
                    self.dndz_source[i], x=self.zz, axis=-1
                )
        else:
            self.n_source = 0
    ...
    def __call__(
        ...
        for i, chi in enumerate(self.chival):
            kval = (ell + 0.5) / chi
            Ccmbkcmbk[:, i] = self.w_cmbk[i] ** 2 / chi**2 * Pmm.P(self.zval[i], kval)
        ...
        if self.n_lens > 0:
            ...
            if self.n_source > 0:
                Cdk = simps(Cdk, x=self.chival, axis=-1).reshape(
                    -1, self.n_lens * self.n_source
                )
            else:
                Cdk = None
        else:
        ...
        if self.n_source > 0:
            Ckk = simps(Ckk, x=self.chival, axis=-1).reshape(
                -1, self.n_source * self.n_source
            )
        else:
            Ckk = None
        ...
        lval = np.arange(Lmax)
        if Cdd is not None:
            Cdd = interp1d(ell, Cdd, axis=0, kind="cubic", fill_value="extrapolate")(
                lval
            ).reshape(-1, self.n_lens)
        if Cdk is not None:
            Cdk = interp1d(ell, Cdk, axis=0, kind="cubic", fill_value="extrapolate")(
                lval
            ).reshape(-1, self.n_lens * self.n_source)
        if Ckk is not None:
        ...
        if Cdcmbk is not None:
            ...
            )(lval).reshape(-1, self.n_lens)
        if Ccmbkcmbk is not None:
            Ccmbkcmbk = interp1d(
                ell, Ccmbkcmbk, axis=0, kind="cubic", fill_value="extrapolate"
            )(lval)
